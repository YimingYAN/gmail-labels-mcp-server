...
function formatLabel(label: GmailLabel): string {
  ...
  if (label.type) parts.push(`  type: ${label.type}`);
  if (label.messagesTotal !== undefined) parts.push(`  messages: ${label.messagesTotal} (${label.messagesUnread || 0} unread)`);
  return parts.join("\n");
}
    ...
    async ({ query, add_label_ids, remove_label_ids, max_results }) => {
      try {
        ...
        const results = await Promise.allSettled(
          messages.map((m) =>
            modifyMessageLabels(client, m.id, add_label_ids ?? [], remove_label_ids ?? [])
          )
        );
